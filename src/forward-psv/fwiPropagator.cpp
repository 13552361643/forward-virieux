...
void fwiPropagator::propagateForward(fwiModel &_currentModel, fwiShot &_shot, bool exportSnapshots, bool performFWI,
    ...
    for (uword it = 0; it < _currentModel.get_nt(); ++it) {
        ...
        if (performFWI and it % _shot.snapshotInterval == 0) {
            ...
            _shot.vxSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) = vx(_currentModel.interiorX, _currentModel.interiorZ);
            _shot.vzSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) = vz(_currentModel.interiorX, _currentModel.interiorZ);
            _shot.txxSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) = txx(_currentModel.interiorX,
                                                                                                  _currentModel.interiorZ);
            _shot.tzzSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) = tzz(_currentModel.interiorX,
                                                                                                  _currentModel.interiorZ);
            _shot.txzSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) = txz(_currentModel.interiorX,
                                                                                                  _currentModel.interiorZ);
        }
...
#pragma omp parallel for collapse(1)
        for (uword receiver = 0; receiver < _shot.receivers.n_rows; ++receiver) {
            uword ix = _shot.receivers.row(receiver)[0] + _currentModel.np_boundary;
            uword iz = static_cast<uword>(_shot.receivers.row(receiver)[1]);
...
#pragma omp parallel for collapse(1)
        ...
        for (uword source = 0; source < _shot.source.n_rows; ++source) {
            uword ix = _shot.source.row(source)[0] + _currentModel.np_boundary;
            uword iz = static_cast<uword>(_shot.source.row(source)[1]);
            ...
            switch (_shot.sourceType) {
                ...
                case fwiShot::momentSourceHeaviside:
                    if (it * _currentModel.get_dt() >= 0.001) {
                        ...
                        vx(ix, iz) +=
                                _shot.moment(0, 0) * _currentModel.get_dt() * _currentModel.b_vz(ix, iz) / (dx * dx * dx * dx);

                        // (z,z)-couple
                        vz(ix, iz - 1) -=
                                _shot.moment(1, 1) * _currentModel.get_dt() * _currentModel.b_vz(ix, iz - 1) / (dz * dz * dz * dz);
                        vz(ix, iz) +=
                                _shot.moment(1, 1) * _currentModel.get_dt() * _currentModel.b_vz(ix, iz) / (dz * dz * dz * dz);

                        // (x,z)-couple
                        vx(ix - 1, iz + 1) +=
                                0.25 * _shot.moment(0, 1) * _currentModel.get_dt() * _currentModel.b_vz(ix - 1, iz + 1) / (dx * dx * dx * dx);
                        vx(ix, iz + 1) +=
                                0.25 * _shot.moment(0, 1) * _currentModel.get_dt() * _currentModel.b_vz(ix, iz + 1) / (dx * dx * dx * dx);
                        vx(ix - 1, iz - 1) -=
                                0.25 * _shot.moment(0, 1) * _currentModel.get_dt() * _currentModel.b_vz(ix - 1, iz - 1) / (dx * dx * dx * dx);
                        vx(ix, iz - 1) -=
                                0.25 * _shot.moment(0, 1) * _currentModel.get_dt() * _currentModel.b_vz(ix, iz - 1) / (dx * dx * dx * dx);
                        ...
                        // (z,x)-couple
                        vz(ix + 1, iz - 1) +=
                                _shot.moment(1, 0) * _currentModel.get_dt() * _currentModel.b_vz(ix + 1, iz - 1) / (dz * dz * dz * dz);
                        vz(ix + 1, iz) +=
                                _shot.moment(1, 0) * _currentModel.get_dt() * _currentModel.b_vz(ix + 1, iz) / (dz * dz * dz * dz);
                        vz(ix - 1, iz - 1) -=
                                _shot.moment(1, 0) * _currentModel.get_dt() * _currentModel.b_vz(ix - 1, iz - 1) / (dz * dz * dz * dz);
                        vz(ix - 1, iz) -=
                                _shot.moment(1, 0) * _currentModel.get_dt() * _currentModel.b_vz(ix - 1, iz) / (dz * dz * dz * dz);
...
void fwiPropagator::propagateAdjoint(fwiModel &_currentModel, fwiShot &_shot, mat &_denistyKernel, mat &_muKernel,
    ...
    // Create taper matrix
    mat taper = _currentModel.np_boundary * ones(static_cast<const uword>(nx), static_cast<const uword>(nz));
    for (uword iTaper = 0; iTaper < _currentModel.np_boundary; ++iTaper) {
        taper.submat(iTaper, 0, nx - iTaper - 1, nz - iTaper - 1) =
                1 + iTaper * ones(nx - 2 * iTaper, nz - iTaper);
    }
    ...
    for (int it = _currentModel.get_nt() - 1; it >= 0; --it) {
        ...
        if (it % _shot.snapshotInterval == 0) {
            mat f1 = _shot.vxSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) %
                     vx(_currentModel.interiorX, _currentModel.interiorZ);

            mat f2 = _shot.vzSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) %
                     vz(_currentModel.interiorX, _currentModel.interiorZ);

            _denistyKernel -=
                    _shot.snapshotInterval * _currentModel.get_dt() * (f1 + f2);

            // Compute adjoint strain using Hooke's Law
            ...
            // Compute regular strain using Hooke's Law
            mat exx = (_shot.txxSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) -
                       (_shot.tzzSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) %
                        _currentModel.c13(_currentModel.interiorX, _currentModel.interiorZ)) /
                       _currentModel.c11(_currentModel.interiorX, _currentModel.interiorZ)) /
                      (_currentModel.c11(_currentModel.interiorX, _currentModel.interiorZ) -
                       (square(_currentModel.c13(_currentModel.interiorX, _currentModel.interiorZ)) /
                        (_currentModel.c11(_currentModel.interiorX, _currentModel.interiorZ))));
            mat ezz = (_shot.tzzSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) -
                       (_shot.txxSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) %
                        _currentModel.c13(_currentModel.interiorX, _currentModel.interiorZ)) /
                       _currentModel.c11(_currentModel.interiorX, _currentModel.interiorZ)) /
                      (_currentModel.c11(_currentModel.interiorX, _currentModel.interiorZ) -
                       (square(_currentModel.c13(_currentModel.interiorX, _currentModel.interiorZ)) /
                        (_currentModel.c11(_currentModel.interiorX, _currentModel.interiorZ))));
            mat exz = _shot.txzSnapshots.slice(static_cast<const uword>(it / _shot.snapshotInterval)) /
                      (2 * _currentModel.c55(_currentModel.interiorX, _currentModel.interiorZ));
